...
def get_covid_data(country=None, timeout=20):
    ...
    cases_raw = timeline.get("cases") or {}
    deaths_raw = timeline.get("deaths") or {}
    recovered_raw = timeline.get("recovered") or {}
...
def plot_covid_graph(records):
    ...
    ax.plot(dates, cases, label="Cases")
    ax.plot(dates, deaths, label="Deaths")
    ax.plot(dates, recovered, label="Recovered")
    ax.set_xlabel("Date")
    ax.set_ylabel("Total")
