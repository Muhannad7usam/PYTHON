...
def create_app(app: Flask):
    ...
    def exchange():
        ...
        csv_download_url = None
        error = None
        comparison_codes = None
        comparison_rows = None

        if request.method == "POST":
            base = (request.form.get("base") or "USD").upper().strip()
            code1 = (request.form.get("code1") or "").upper().strip()
            code2 = (request.form.get("code2") or "").upper().strip()
            code3 = (request.form.get("code3") or "").upper().strip()

            raw_codes = [c for c in [code1, code2, code3] if c]
            symbols_str = ",".join(raw_codes) if raw_codes else None

            resp = get_exchange_data(base=base, symbols=symbols_str or None)
            if isinstance(resp, dict) and resp.get("error"):
                ...
                if rows:
                    ...
                    if raw_codes:
                        code_to_rate = {r["code"]: r["rate"] for r in rows if r.get("code")}
                        comparison_codes = [c for c in raw_codes if c in code_to_rate]
                        if len(comparison_codes) >= 2:
                            comparison_rows = []
                            for from_code in comparison_codes:
                                from_rate = code_to_rate.get(from_code)
                                row_values = []
                                for to_code in comparison_codes:
                                    to_rate = code_to_rate.get(to_code)
                                    if from_code == to_code:
                                        val = 1.0
                                    else:
                                        if from_rate and to_rate:
                                            val = to_rate / from_rate
                                        else:
                                            val = None
    ...
    def weather():
        ...
        if request.method == "POST":
            ...
            active_locations = [
                ...
                if loc["display_name"] or loc["lat"] or loc["lon"]
            ]

            if not active_locations:
                error_msg = "Please enter at least one location (by name or by latitude and longitude)."
            else:
                for loc in active_locations:
                    ...
                    if isinstance(res, dict) and res.get("error"):
                        label = name or f"{lat},{lon}" if (lat and lon) else "Location"
                        if error_msg:
                            error_msg = f"{error_msg} {label}: {res['error']}"
                        else:
                            error_msg = f"{label}: {res['error']}"
                    else:
                        weather_data.append(res)

                if weather_data and not error_msg:
                    ...
                    if save_res.get("ok"):
                        ...
                        error_msg = save_res.get("error")

            locations = weather_data if weather_data else raw_locations
